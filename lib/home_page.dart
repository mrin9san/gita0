...
import 'trainer_list_page.dart'; // NEW
...
class _HomePageState extends State<HomePage> {
  ...
  void _navigateToTrainerList() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => TrainerListPage(fireBaseId: widget.fireBaseId),
      ),
    );
  }
  ...
  // ======= Add Trainer dialog (multi-gym attach) =======
  void _showAddTrainerForm() async {
    final formKey = GlobalKey<FormState>();
    ...
    // gym multi-select: only those with real GymID
    final gymsWithId = userGyms
        .where(
            (g) => (g['GymID'] is String) && (g['GymID'] as String).isNotEmpty)
        .toList();
              ...
              child: Form(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 10),
                    _glassyField(controller: qualC, label: 'Qualification'),
                    const SizedBox(height: 10),
                    _glassyField(
                      controller: heightC,
                      label: 'Height (cm)',
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  final payload = {
                    // IMPORTANT: explicitly null GymID to avoid default FK issue
                    'GymID': null,
                    'FirebaseID': widget.fireBaseId,
                    'Name': nameC.text.trim(),
                    'Age': _i(ageC),
                  ...
                  if (trainerId != null && trainerId.isNotEmpty) {
                    ...
                    if (gymIds.isNotEmpty) {
                      // bulk insert links
                      await _client.from('TrainerGyms').insert([
        ...
        child: userGyms.isEmpty
            ...
            : ListView(
                ...
                children: [
                  ...
                  SizedBox(
                    height: 160,
                    child: _buildTrainersCard(context),
                  ),
                ],
              ),
      ),
  ...
  Widget _buildTrainersCard(BuildContext context) {
    // Match Gym card geometry
    ...
    final actions = <_ActionSpec>[
      _ActionSpec(
        ...
        onTap: _showAddTrainerForm,
      ),
      _ActionSpec(
        icon: Icons.groups_2_outlined,
        label: 'See Trainers',
        bg: Colors.purpleAccent,
        onTap: _navigateToTrainerList,
      ),
    ];
      ...
      child: Padding(
        ...
        child: LayoutBuilder(
          builder: (context, c) {
            ...
            final pinnedTrainersLabel = Positioned(
              left: 12,
              top: 12,
              child: Row(
                children: const [
